...
function useMetaMaskEthersSignerInternal(parameters: { initialMockChains?: Readonly<Record<number, string>> }): UseMetaMaskEthersSignerState {
  ...
  const { provider, chainId, accounts, isConnected, connect, error, isConnecting } = useMetaMask();
  ...
  const sameSigner = useRef((ethersSigner: ethers.JsonRpcSigner | undefined) => ethersSigner === ethersSignerRef.current);
  ...
  return { sameChain, sameSigner, provider, chainId, accounts, isConnected, connect, isConnecting, ethersBrowserProvider, ethersReadonlyProvider, ethersSigner, error, initialMockChains };
}
